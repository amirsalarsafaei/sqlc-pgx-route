...
import (
	...
	"github.com/amirsalarsafaei/sqlc-pgx-route/internal/opts"
...
func (i *importer) Imports(filename string) [][]ImportSpec {
	...
	case "poolroute.go":
		return mergeImports(i.poolrouteImports())
	default:
...
func (i *importer) poolrouteImports() fileImports {
	...
	// Scan all queries for types used in method signatures
	for _, q := range i.Queries {
		if q.Cmd == metadata.CmdCopyFrom || strings.HasPrefix(q.Cmd, ":batch") {
			continue
		}
		if q.Cmd == metadata.CmdExecResult {
			switch sqlpkg {
	...
	// Check types used in query arg/return signatures
	...
	for typeName, stdPkg := range stdlibTypes {
		if uses(typeName) {
			std[stdPkg] = struct{}{}
		}
	}
